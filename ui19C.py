...
class Application19C(tk.Tk):
    ...
    def set_label_widgets(self):
        ...
        self.filiere_label = tk.Label(self, text="Filière ou type d'installation ", bg=COLOR1, fg=COLOR2, font=labels_font)
        ...
        self.titulaire_label = tk.Label(self, text="Code du titulaire du contrat ", bg=COLOR1, fg=COLOR2, font=labels_font)
        self.titulaire_label.grid(row=5, column=1, sticky='E')
        self.service_label = tk.Label(self, text="Bigramme du service destinataire ", bg=COLOR1, fg=COLOR2, font=labels_font)
    ...
    def set_combobox_widgets(self):
        ...
        self.classement_combobox = ttk.Combobox(self, values=self.db.classements, width=combobox_width, state='readonly')
    ...
    def set_button_widgets(self):
        self.generate_bt = tk.Button(self, text='Générer le 19C', font=('Arial', 25, 'bold'), width=30, command=self.generate_19C)
    ...
    def set_menu(self):
        ...
        menu_references_19C.add_command(label="Ajouter un IP auteur", command=self.open_add_user_Window)
        menu_bar.add_cascade(label='Options', menu=menu_references_19C)
        self.config(menu=menu_bar)

    def generate_19C(self):
        mandatory_fields = [self.filiere_combobox.current(), self.site_combobox.current(), self.classement_combobox.current(),
                            self.contrat_combobox.current(), self.titulaire_combobox.current(), self.service_combobox.current(),
                            self.type_combobox.current(), self.auteur_combobox.current()]
        if -1 in mandatory_fields:
            messagebox.showerror(title='Données insuffisantes', message='Tous les champs avec un * doivent être renseignés')
        else:
            ...
            db_new_19C_entry_tuple = [self.my19C.reference, self.my19C.titre, str(datetime.datetime.utcnow().now())[:19],
            ...
            if self.my19C.chrono == '000':
                ...
                confirmation_message = f"La référence {self.my19C.reference} va être enregistré dans la base de donnée.\n\n" \
                                       f"Note : pour éviter les références jumelles, le numéro chrono a été incrémenté à la valeur {self.my19C.chrono}.\n\n" \
                                       f"Confirmez-vous l'exactitude de vos données d'entrée ?"
    ...
    def open_ref_table_Window(self):
            ...
            columns = ('ref_id', 'code', 'titre', 'date', 'actif', 'auteur', 'commentaire')
            columns_widths = (40, 160, 160, 160, 40, 80, 400)
            refs_table = ttk.Treeview(refs_win, columns=columns, show='headings', selectmode='extended', height=20)
            ...
            for row in self.db.references_19C:
